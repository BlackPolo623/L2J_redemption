...
public class ClassMaster extends Merchant
{
	...
	public void onBypassFeedback(Player player, String command)
	{
		...
		else if (command.startsWith("increase_clan_level"))
		{
			...
			else if (player.getClan().getLevel() >= 11)
			{
				final NpcHtmlMessage html = new NpcHtmlMessage(getObjectId());
		...
		else if (command.startsWith("clan_skill"))
		{
		final Clan clan = player.getClan();
		if(clan != null){
		if(player.isClanLeader()){
		final Map<Integer, SkillLearn> skills = SkillTreeData.getInstance().getMaxPledgeSkills(clan, true);
