		...
		synchronized (item)
		{
			...
			switch (resultType)
			{
				...
				case SUCCESS:
				{
					...
					{
						if(Config.ENCHANT_ZHUFU){
							item.setEnchantLevel(item.getEnchantLevel() + Config.ENCHANT_ZHUFUCHENGGONG);
						}else{
				...
				case FAILURE:
				{
					...
					else
					{
						...
						if (scrollTemplate.isBlessed())
						{
							...
							// blessed enchant - clear enchant value
							if(Config.ENCHANT_ZHUFU && (item.getId() < 24128 || item.getId() > 24135)){
								item.setEnchantLevel(item.getEnchantLevel() - Config.ENCHANT_ZHUFUSHIBAI);
								player.sendMessage("强化失败，相关道具强化值减"+ Config.ENCHANT_ZHUFUSHIBAI+".");
							}else{
